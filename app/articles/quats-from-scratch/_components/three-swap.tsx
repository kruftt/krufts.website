...
export default function ThreeSwapper({ className }: { className?: string }) {
	...
	const update = useCallback(() => {
		const _p1 = p1.current;
		const _p2 = p2.current;
		if (!_p1 || !_p2) return;
		_p1.style.transform = outer_translations[state.p1];
		_p2.style.transform = inner_circles[state.p2];
	}, [state.p1, state.p2]);
	...
	return (
		<div className={`select-none ${className}`}>
			...
			<svg
				...
				viewBox="-61 -45 122 90"
			>
				<title>A diagram of quaternion components.</title>
				<defs>
					<marker
						id="arrow_head"
						viewBox="0 0 2 2"
				...
				<circle
					ref={p1}
					className="transition-transform duration-200 opacity-[0.8]"
					r="6"
					stroke="#555"
					strokeWidth={0.3}
					fill="#e3df04"
					transform={outer_translations[state.p1]}
				></circle>
				<circle
					ref={p2}
					className="transition-transform duration-200 opacity-[0.8]"
					...
					strokeWidth={0.3}
					fill="#d67405"
					transform={inner_circles[state.p2]}
				></circle>
				...
				<text
					...
					fontSize="7"
					fontWeight={400}
					transform={outer_translations[0]}
				>
					i
				</text>
				<text
					...
					fontSize="7"
					fontWeight={400}
					transform={outer_translations[1]}
				>
					-1
				</text>
				<text
					...
					fontSize="7"
					fontWeight={400}
					transform={outer_translations[2]}
				>
					-i
				</text>
				<text
					...
					fontSize="7"
					fontWeight={400}
					transform={outer_translations[3]}
				>
					1
				</text>
				<text
					...
					fontSize="7"
					fontWeight={400}
					transform={inner_translations[0]}
				>
					j
				</text>
				<text
					...
					fontSize="7"
					fontWeight={400}
					transform={inner_translations[1]}
				>
					k
				</text>
				<text
					...
					fontSize="7"
					fontWeight={400}
					transform={inner_translations[2]}
				>
					-j
				</text>
				<text
					...
					fontSize="7"
					fontWeight={400}
					transform={inner_translations[3]}
				>
					-k
				</text>
			</svg>
			<div className="flex justify-center gap-3 mt-8">
				<button
					type="button"
					className={cn(
						"border border-gray-400 pt-1 pb-1 pl-3 pr-3 rounded-lg bg-primary-foreground hover:bg-accent active:bg-muted cursor-pointer",
					)}
				...
				<button
					type="button"
					className={cn(
						"border border-gray-400 pt-1 pb-1 pl-3 pr-3 rounded-lg",
						"bg-primary-foreground hover:bg-accent active:bg-muted cursor-pointer",
					)}
