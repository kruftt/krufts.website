...
const ONE_OVER_ROOT_TWO = Math.SQRT1_2;
...
class Visualizer {
	...
	constructor(canvas: HTMLCanvasElement) {
		...
		const camera = new THREE.PerspectiveCamera(67, 1.33, 0.01, 4);
		...
		this.vec_0 = new VisualizerVector(
			scene,
			m_vector,
			this.q_0,
			this.p_0,
		...
		this.vec_1 = new VisualizerVector(
			scene,
			m_vector,
			this.q_1,
			this.p_1,
		...
		this.vec_2 = new VisualizerVector(
			scene,
			m_vector,
			this.q_2,
			this.p_2,
	...
	update() {
		...
		this.a_1.update(half_cos, half_sin);
		this.a_2.update(cos, sin);
		this.vec_0.update(0, 0);
		this.vec_1.update(half_cos, half_sin);
		this.vec_2.update(0, sin);
...
class VisualizerVector {
		...
		if (ctx) {
			...
			this.label = new THREE.Mesh(
				new THREE.PlaneGeometry(
					ctx.width * TEXT_SCALE,
					ctx.height * TEXT_SCALE,
				),
				mat,
			);
	...
	update(sin: number, cos: number) {
		...
		if (_l) {
			...
			_l.position.set(this.p.x * ratio, this.p.y * ratio, this.p.z * ratio);
		}
	}
}
...
class VisualizerArc {
	...
	update(theta: number) {
		...
		const rotor = new THREE.Quaternion(
			0,
			Math.sin(d_theta),
			0,
			Math.cos(d_theta),
		...
		let _p1: THREE.Vector3, _p2: THREE.Vector3;
		_p2 = this.points[0];
		quatToPoint(r, _p2);
		if (reverse) {
			r.multiply(rotor);
...
export default function QuaternionVisualizer() {
	...
	useEffect(() => {
		...
		if (!_canvas) return;
		visualizer.current = new Visualizer(_canvas);
	...
	const updateTheta = useCallback((e: ChangeEvent<HTMLInputElement>) => {
		...
		const _visualizer = visualizer.current;
		if (!_visualizer) return;
		_visualizer.setTheta(v);
	...
	return (
		<div className="select-none rounded-xl bg-[#FFF] text-[#111]">
			...
			<MathBlock>
				{
					"q = \\cos{(\\frac{\\theta}{2})} + \\hat{q}\\sin{(\\frac{\\theta}{2})}"
				}
			</MathBlock>
