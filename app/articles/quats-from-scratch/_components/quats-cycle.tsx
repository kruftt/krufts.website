...
export function QuatsCycle({ children }: {
  ...
  const pause = useCallback(() => {
    ...
    const container = item_container.current;
    if (!container) return;
    const _children = container.children as unknown as HTMLImageElement[]
...
function PlayPause({ paused }: { paused: boolean }) {
  return (
			<svg viewBox="0 0 10 10">
				...
				<path
					d="M 3 1 L 3 9 M 7 1 L 7 9"
					stroke="#202020"
					strokeWidth="2"
					visibility={paused ? "hidden" : "visible"}
				></path>
				<path
					d="M 3 1 L 3 9 L 9 5 L 3 1"
					stroke="#202020"
					fill="#202020"
					strokeWidth="0"
