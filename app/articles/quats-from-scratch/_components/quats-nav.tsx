...
export default function QuatsNav({
	...
	return (
		<div className={cn("flex flex-col items-center", className)}>
			{header && (
				<h1 className={"pl-3 pr-3 rounded-md text-3xl mb-8"}>
			...
			<div className="flex gap-2 justify-center text-lg">
				...
				<LinkButton href="/articles/quats-from-scratch/" selected={name === ""}>
					Intro
				</LinkButton>
				<LinkButton
					href="/articles/quats-from-scratch/0"
					selected={name === "0"}
				>
					0
				</LinkButton>
				<LinkButton
					href="/articles/quats-from-scratch/1"
					selected={name === "1"}
				>
					1
				</LinkButton>
				<LinkButton
					href="/articles/quats-from-scratch/2"
					selected={name === "2"}
				>
					2
				</LinkButton>
				<LinkButton
					href="/articles/quats-from-scratch/3"
					selected={name === "3"}
				>
					3
				</LinkButton>
	...
	return (
		<div
			className={cn(
				...
				selected
					? "outline outline-muted bg-card/80 shadow-sm"
					: "hover:outline hover:outline-muted hover:bg/80-card cursor-pointer",
			)}
		>
			{(selected && (
				<span className="h-full block pl-3 pr-3">{children}</span>
			)) || (
				<Link className="h-full block pl-3 pr-3" href={href}>
					{children}
				</Link>
			)}
		</div>
...
function StepButton({
	...
	return (
		<div className="rounded-md flex items-center select-none opacity-[0.9] hover:opacity-[1] hover:outline hover:outline-gray-300 hover:bg-white">
			<Link className="block pl-2 pr-2" href={href}>
				{children}
			</Link>
		</div>
	);
