...
export default function Swapper({ className }: { className?: string }) {
	...
	const bottom_translations = useMemo(
		() => [
			"translate(-39.0, 0.0)",
			"translate(-13.0, 0.0)",
			"translate(13.0, 0)",
	...
	const [positions, setPositions] = useState<[string, string, string, string]>([
		translations[0],
		translations[1],
		translations[2],
		translations[3],
	...
	>([
		bottom_translations[0],
		bottom_translations[1],
		bottom_translations[2],
		bottom_translations[3],
	]);
	...
	return (
		<div className={`select-none ${className}`}>
			<svg
				ref={svg}
				className="m-auto w-full max-w-90 aspect-13/10"
				viewBox="-55 -55 110 143"
			>
				<title>
					Four dots arranged at the points of a cross, in which dots/elements
					can be mutated either by swapping the left and right elements (as if
					rotating on the vertical axis), or by simultaneously swapping the top
					with the right and the bottom with the left (as if rotating around the
					diagonal axis).
				</title>
				<defs>
					<marker
