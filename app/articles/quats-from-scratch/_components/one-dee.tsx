...
export default function OneDee({ className }: { className?: string }) {
	...
	const update = useCallback(() => {
		const _modulus = modulus.current;
		const _svg = svg.current;
		if (!_modulus || !_svg) return;

		const slider_value = parseFloat(_modulus.value);
	...
	return (
		<div className={`select-none ${className}`}>
			<svg ref={svg} className="w-full h-10">
				<title>An arrow that can change its length and direction.</title>
				<defs>
					<marker
						id="triangle"
						viewBox="0 0 2 2"
			...
			<JsdImage
				width={599}
				height={154}
				src="quats/scalar_action.png"
				className="max-w-7/8 max-h-11 m-auto mt-6! mb-6!"
